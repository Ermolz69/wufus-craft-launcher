...
use crate::infrastructure::fs::{PathResolver, FsManager};

#[tauri::command]
pub fn initialize_fs(app_handle: AppHandle, state: State<SettingsState>) -> Result<(), LauncherError> {
...
pub fn save_settings(
    ...
    let path = get_settings_path(&app_handle).map_err(|e| LauncherError::SystemError(e))?;
    save_to_disk(&path, &settings).map_err(|e| LauncherError::SettingsWriteError(e))?;
...
#[tauri::command]
pub fn reset_settings(app_handle: AppHandle, state: State<SettingsState>) -> Result<LauncherSettings, LauncherError> {
    let _span = info_span!("settings", task="reset").entered();
    let default_settings = LauncherSettings::default();
    let path = get_settings_path(&app_handle).map_err(|e| LauncherError::SystemError(e))?;
    save_to_disk(&path, &default_settings).map_err(|e| LauncherError::SettingsWriteError(e))?;
...
pub fn open_logs_folder(app_handle: tauri::AppHandle) -> Result<(), LauncherError> {
    ...
    if !logs_dir.exists() {
        std::fs::create_dir_all(&logs_dir).map_err(|e| LauncherError::SystemError(format!("Failed to create logs dir: {}", e)))?;
    }

    open::that(logs_dir).map_err(|e| LauncherError::SystemError(format!("Failed to open logs folder: {}", e)))?;
    Ok(())
}
