...
impl LocalStateStore {
    pub fn load(paths: &StatePaths) -> LocalState {
        ...
        let json = match fs::read_to_string(&file_path) {
            Ok(j) => j,
            Err(e) => {
                error!("Failed to read state file {:?}: {}", file_path, e);
                return LocalState::default();
            }
        };
        ...
        let state: LocalState = match serde_json::from_str(&json) {
            Ok(s) => s,
            Err(e) => {
                error!("Failed to parse state file {:?}: {}", file_path, e);
                return LocalState::default();
            }
        };

        if let Err(e) = StateValidator::validate(&state) {
            warn!("State validation failed: {}. Re-creating state.", e);
            return LocalState::default();
        }
    ...
    pub fn save(paths: &StatePaths, state: &LocalState) -> Result<(), LauncherError> {
        ...
        let json = serde_json::to_string_pretty(state).map_err(|e| {
            LauncherError::SystemError(format!("Failed to serialize state: {}", e))
        })?;

        fs::write(&file_path, json).map_err(|e| {
            LauncherError::SystemError(format!("Failed to write state file: {}", e))
        })?;
